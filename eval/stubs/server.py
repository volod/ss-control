"""Tiny composition stand-ins: Streamlit, Frigate, fusion-rt incidents, registry."""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path in {"/healthz", "/health"}:
            self._send(200, b"ok")
            return
        if path.startswith("/streamlit") or path.startswith("/frigate"):
            if not _authed(self):
                self._send(401, b"unauthorized")
                return
            app = "streamlit" if "streamlit" in path else "frigate"
            html = f"<html><body>{app} ok user={_header(self, 'Remote-User')}</body></html>"
            self._send(200, html.encode("utf-8"), "text/html")
            return
        if path.startswith("/fusion"):
            if not _header(self, "Authorization").lower().startswith("bearer "):
                self._send(401, b"unauthorized")
                return
            payload = {"incidents": [{"id": "inc-1", "severity": "high", "source": "eval"}]}
        ...
        if path.startswith("/registry"):
            if not _header(self, "Authorization").lower().startswith("bearer "):
                self._send(401, b"unauthorized")
                return
            payload = {"devices": [{"id": "dev-1", "kind": "sensor"}]}
            self._send(200, json.dumps(payload).encode("utf-8"), "application/json")
            return
